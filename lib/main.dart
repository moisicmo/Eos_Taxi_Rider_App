...
Future<void> requestNotificationPermission() async {
  ...
  if (Platform.isAndroid) {
    ...
    if(apiLevel > 32) {
      print("Api Level " + apiLevel.toString());
      final permission = Permission.notification;
      if (await permission.isDenied) {
        final result = await permission.request();
        if (result.isGranted) {
          oneSignalSettings(permissionGranted: "granted");
        } else if (result.isDenied) {
          oneSignalSettings(permissionGranted: "denied");
        } else if (result.isPermanentlyDenied) {
          oneSignalSettings(permissionGranted: "permanently_denied");
        }
      }
    } else {
      oneSignalSettings(permissionGranted: "unknown");
    }
  } else if (Platform.isIOS) {
    IosDeviceInfo iosInfo = await deviceInfo.iosInfo;
    // Örnek olarak, sistem sürümünü analiz ederek API seviyesini çıkarıyoruz.
    List<String> versionComponents = iosInfo.systemVersion.split('.');
    apiLevel = int.parse(versionComponents.first);
    oneSignalSettings(permissionGranted: "unknown");
  } else {
    throw UnsupportedError('Bu işletim sistemi desteklenmiyor.');
  }
}
