...
class DashBoardScreenState extends State<DashBoardScreen> {
  ...
  Future<void> locateAndAnimateToUserPosition() async {
    // Kullanıcı konumunu al (varsayılan olarak driverLocation kullanılır, yoksa sharedPref'ten alınır)
    LatLng targetPosition = sourceLocation ?? LatLng(sharedPref.getDouble(LATITUDE)!, sharedPref.getDouble(LONGITUDE)!);

    // Haritayı kullanıcının konumuna animasyonlu olarak taşıyacak CameraPosition
    CameraPosition cameraPosition = CameraPosition(target: targetPosition, zoom: 17.0);

    // _controller.future ile GoogleMapController'a eriş
    final GoogleMapController mapController = await _controller.future;

    // animateCamera ile haritayı cameraPosition'a animasyonlu olarak taşı
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Container(
            margin: const EdgeInsets.only(left: 355.0, right: 0.0,top: 430),
            child: new SizedBox(
              width: 40.0,
              height: 40.0,
              child: ElevatedButton(
                child: Icon(Icons.location_pin),
                onPressed: locateAndAnimateToUserPosition,
